...
@Service
public class ProcessBusinessBefore {
    @Resource
    ActionBusinessBefore actionBusinessBefore;
//    @Autowired
//    private FlowExecutor flowExecutor;
    ...
    public Map<String, Action> getProcessActions(String processId) {
        ...
        actionMetaList.forEach(actionMeta -> {
            try {
                System.out.println(actionMeta);
                Action action = actionBusinessBefore.getAction(actionMeta.getType(), actionMeta.getObjectId(), actionMeta.getExecParam());
                    ...
                    if (action != null) {
                        ...
                        if (inputParam == null || inputParam.equals("")){
                            if (executeArgs == null || executeArgs.isEmpty()){
                                output = actionBusinessBefore.executeAction(action, actionMeta.getExecParam());
                            } else {
                                output = actionBusinessBefore.executeAction(action, actionMeta.getExecParam(), executeArgs);
                            }
                        }else {
...
//                                System.out.println("父节点"+parentActionId+"的输出"+inputParam);
                                output = actionBusinessBefore.executeAction(action, actionMeta.getExecParam(), Integer.parseInt(inputParam));
...
//                                System.out.println("父节点"+parentActionId+"的输出"+inputParam);
                                ...
                                output = actionBusinessBefore.executeAction(action, actionMeta.getExecParam(), parentOutput.getOutputParams());
                            } else {
...
//            ActionMeta actionMeta1 = new ActionMeta("makeCoffee", "制作咖啡", "Device", "", "CoffeeMaker", "makeCoffee", null, "");
            //ActionMeta actionMeta2 = new ActionMeta("check", "检查", "Device", "makeCoffee", "CoffeeMaker", "check", null, "");
//            return new ArrayList<>(Arrays.asList(actionMeta1));
        }
